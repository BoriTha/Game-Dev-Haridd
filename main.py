...
from src.core.utils import draw_text, get_font, resource_path, resource_path
...
class Game:
    def __init__(self):
        ...
        try:
            self.arrow_sprite = pygame.image.load(resource_path("assets/Player/Ranger/arrow.png")).convert_alpha()
        ...
        self.bg_tile = None
        try:
            self.bg_tile = pygame.image.load(resource_path(BACKGROUND_IMAGE_PATH)).convert()
            # Force size 64x64
    ...
    def restart_run(self):
        """
        Restart from the first level (reset progression on death).
        """
        # Reset to first level on death
        if self.use_pcg:
            level_to_restart = 1  # PCG levels are 1-based
            self.current_level_number = 1
        else:
            level_to_restart = 0  # Legacy levels are 0-based
            self.level_index = 0
        ...
        # Reset camera and restore level boundaries
        self.camera = Camera()
        # Restore camera boundaries from the loaded level
        if hasattr(self.level, 'w') and hasattr(self.level, 'h'):
            self.camera.level_width = self.level.w
            self.camera.level_height = self.level.h

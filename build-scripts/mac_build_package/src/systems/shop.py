...
def _safe_load_icon(path: str, size: tuple = (24,24)) -> Optional[pygame.Surface]:
    """Return loaded surface for the given icon path."""
    if not path:
        return None
    try:
...
class Shop:
    ...
    def _draw_player_slots_column(self, screen, rect):
        ...
        for i in range(3):
            ...
            if item:
                ...
                inner_rect = slot_rect.inflate(-8, -8)
                icon_surf = None
                if hasattr(item, 'icon_path') and item.icon_path:
                ...
                if icon_surf:
                    ...
                    # Fallback: draw colored background and letter
                    pygame.draw.rect(screen, item.color, inner_rect, border_radius=6)
                    if hasattr(item, 'icon_letter'):
    ...
    def _draw_inventory_stock_view(self, screen, rect):
        ...
        for idx, (key, item) in enumerate(items_to_show):
            ...
            # Try to load and draw icon
            inner_rect = item_rect.inflate(-8, -8)
            icon_surf = None
            if hasattr(item, 'icon_path') and item.icon_path:
    ...
    def _draw_player_info_column(self, screen, rect):
        ...
        for i, item in enumerate(stats_lines):
            ...
            if stat_y + line_height >= stats_area_rect.y and stat_y <= stats_area_rect.bottom:
                ...
                if len(item) == 4:
                    text, color, mod_text, mod_color = item[0], item[1], item[2], item[3]
                    # Draw main stat

...
def _safe_load_icon(path: str, size: tuple = (24,24)) -> _Optional[pygame.Surface]:
    ...
    if not surf:
        return None
    # Always return the surface if we successfully loaded it
    # The transparency check was causing icons to not display
    return surf
...
class Inventory:
    ...
    def __init__(self, game):
        ...
        self.armament_order = []  # Player starts with no armaments - they must be discovered
    ...
    def _refresh_inventory_defaults(self):
        ...
        # Clear gear slots on restart - player starts with no gear
        self.gear_slots = [None, None, None]
        self.armament_order = []  # Clear armaments - player must discover them
    ...
    def draw_inventory_overlay(self):
        ...
        for idx in range(3):
            ...
            if item:
                ...
                icon_img = None
                if hasattr(item, 'icon_path') and item.icon_path:
                    icon_img = _safe_load_icon(item.icon_path, (inner.width, inner.height))
                    if not icon_img:
                        icon_img = load_icon_masked(item.icon_path, (inner.width, inner.height), radius=6)
                if icon_img:
                    self.game.screen.blit(icon_img, icon_img.get_rect(center=inner.center))
                else:
                    # Fallback: draw colored rect and letter
                    pygame.draw.rect(self.game.screen, item.color, inner, border_radius=6)
